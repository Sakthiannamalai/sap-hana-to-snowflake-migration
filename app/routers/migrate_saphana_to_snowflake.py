...
async def process_sap_hana_file(request: ConvertFileRequest):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(request.s3_link)

        if response.status_code != 200:
            logger.error(f"Failed to download file from AWS. Status code: {
                         response.status_code}")
            raise HTTPException(
                status_code=400, detail="Failed to download file from AWS.")

        zip_file = io.BytesIO(response.content)
        logger.info("Successfully downloaded file from AWS.")
        ...
        with zipfile.ZipFile(zip_file, "r") as zip_ref:
            ...
            for idx, zip_info in enumerate(file_list, start=1):
                ...
                update_status(request.file_uuid, 'In Progress', f"{
        ...
        folder_key = "converted-snowflake-files/"
        s3_client.put_object(Bucket=AWS_BUCKET_NAME, Key=folder_key)
        logger.info(f"Folder '{folder_key}' created in bucket '{
                    AWS_BUCKET_NAME}'.")

        converted_zip_key = f"converted-snowflake-files/{
            request.file_uuid}.zip"
        s3_client.upload_fileobj(
            zip_output, AWS_BUCKET_NAME, converted_zip_key)
        logger.info("Successfully uploaded the converted ZIP file to S3.")

        download_url = s3_client.generate_presigned_url(
            "get_object",
            Params={"Bucket": AWS_BUCKET_NAME,
                    "Key": converted_zip_key},
            ExpiresIn=PRESIGNED_URL_EXPIRY,
        )
        logger.info(f"Generated presigned URL for the converted file: {
                    download_url}")

        auth_url = "http://ec2-3-85-165-156.compute-1.amazonaws.com:8080/api/v1/authenticate"
        webhook_url = "http://ec2-3-85-165-156.compute-1.amazonaws.com:8080/api/v1/transform/processed-files"
        ...
        async with httpx.AsyncClient() as client:
            ...
            webhook_response = await client.post(webhook_url, json=payload, headers=headers)
